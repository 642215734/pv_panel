...
# 3.formula
def calcu3(mppt):
    mppt_test = mppt.iloc[:, 0:65]
    try:
        mppt_test['mean'] = mppt_test.mean(axis=1)
    except ValueError:
        return None
    mean_value = mppt_test['mean'].max()
    a = mppt_test['mean'] < mean_value * 0.4
    output = mppt[a]
...
if __name__ == '__main__':
    ...
    for i, file in enumerate(file_names):
        ...
        file_name = ''.join(['..\\DB\\output', str(i), '.xls'])
        to_xls(output_all, file_name)

        # file = '../optimizer_data/optimizer_201902_05.csv'
        # output_all = parse_file(file)
        #
        # file_name = '../D_connect/D.xls'
        # to_xls(output_all, file_name)
